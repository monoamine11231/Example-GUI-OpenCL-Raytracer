...
void cl_wrap_init(cl_wrap* wrap, cl_device_type type, const char* filename) {
    ...
    if (clGetPlatformIDs(1, &platform, NULL) < 0) {
        perror("ERROR:\tCannot find a CL platform\n");
        exit(1);
    }

    if (clGetDeviceIDs(platform, type, 1, &wrap->device, NULL) == CL_DEVICE_NOT_FOUND) {
        perror("ERROR:\tCannot find a device of the given type\n");
        exit(1);
    }

    wrap->context = clCreateContext(NULL, 1, &wrap->device, NULL, NULL, &cl_error);
    if (cl_error < 0) {
        perror("ERROR:\t Could not create a CL context from the device\n");
        exit(1);
    }

    
    if ((source_reader = fopen(filename, "r")) == NULL) {
        perror("ERROR:\tCannot open the source file for reading\n");
        exit(1);
    }
    ...
    if (cl_error < 0) {
        perror("ERROR:\tCouldn't load the source code to the compiler\n");
        exit(1);
    }
    ...
    if (clBuildProgram(wrap->program, 0, NULL, NULL, NULL, NULL) < 0) {
        ...
        clGetProgramBuildInfo(wrap->program, wrap->device, CL_PROGRAM_BUILD_LOG,
                              log_size + 1, log, NULL);
        perror("ERROR:\tCouldn't compile the source. Compiler LOG is shown below:\n");
        printf("%s\n", log);
        free(log);
        exit(1);
    }
    ...
    wrap->queue = clCreateCommandQueueWithProperties(wrap->context, wrap->device,
                                                     NULL, &cl_error);
    if (cl_error < 0) {
        perror("ERROR:\tCouldn't create a command queue for the given device\n");
        exit(1);
    }

    wrap->kernel = clCreateKernel(wrap->program, "raytracer", &cl_error);
    if (cl_error < 0) {
        perror("ERROR:\tCouldn't create a CL kernel\n");
        exit(1);
    }
}
...
void cl_wrap_load_global_data(cl_wrap* wrap, cl_uint arg_id, const void* data, 
    ...
    for (cl_uint i = 0; i < wrap->args_num; i++) {
        if (wrap->used_args[i] == arg_id) {
            perror("ERROR:\tGiven kernel argument already in use\n");
            exit(1);
        }
    }

    if (arg_id >= 32) {
        perror("ERROR:\tWrong kernel ID given\n");
        exit(1);
    }
    ...
    if (data) {
        ...
        if (cl_error < 0) {
            perror("ERROR:\tCouldn't transfer the data from host to the device\n");
            exit(1);
        }
    }

    if (clSetKernelArg(wrap->kernel,arg_id,sizeof(cl_mem),&wrap->buffers[arg_id]) < 0) {
        perror("ERROR:\tCouldn't pass the data argument to the kernel\n");
        exit(1);
    }
...
void cl_wrap_output(cl_wrap* wrap, size_t main_size, size_t output_size, cl_int arg_id,
    ...
    if (cl_error < 0) {
        perror("ERROR:\tCannot get the work group size for the given device\n");
        exit(1);
    }
    /* Calculate the global size based on the local one */
    global_size = ceil(main_size/(float)local_size)*local_size;
    ...
    if (cl_error < 0) {
        perror("ERROR:\tCouldn't run the kernel\n");
        exit(1);
    }

    clFinish(wrap->queue);
    /* Transfer the data from the device to the host */
    clEnqueueReadBuffer(wrap->queue, wrap->buffers[arg_id], CL_TRUE, 0, output_size,
